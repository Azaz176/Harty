...
const sortSchema = z
  .enum(["relevance", "newest", "price_asc", "price_desc", "discount", "popularity"])
  .default("relevance");
...
function applyFilters(
  ...
  if (filters.brands?.length) {
    const brandIds = brands.filter((b) => filters.brands?.includes(b.slug)).map((b) => b.id);
    filtered = filtered.filter((p) => brandIds.includes(p.brandId));
  }
  if (filters.categories?.length) {
    const catIds = categories.filter((c) => filters.categories?.includes(c.slug)).map((c) => c.id);
    filtered = filtered.filter((p) => catIds.includes(p.categoryId));
  }
  if (filters.sizes?.length) {
    filtered = filtered.filter((p) =>
      p.variants.some((v) => filters.sizes?.includes(v.size) && v.inStock),
    );
  }
  if (filters.colors?.length) {
    filtered = filtered.filter((p) =>
      p.variants.some((v) => filters.colors?.includes(v.colorName.toLowerCase())),
    );
  }
...
function computeFacets(
  ...
  const priceBuckets = [
    ...
    ...bucket,
    count: filtered.filter((p) => p.basePrice >= bucket.min && p.basePrice <= bucket.max).length,
  }));
  ...
  return {
    ...
    sizes: [
      ...
      "200ml",
      "One Size",
    ].map((s) => ({ value: s, count: countSizes.get(s) ?? 0 })),
    colors: [
      "black",
      "white",
      "navy",
...
export const catalogRouter = router({
  ...
  product: publicProcedure.input(z.object({ slug: z.string() })).query(({ input }) => {
  ...
  related: publicProcedure
    ...
    .query(({ input }) => {
      ...
      switch (input.kind) {
        case "similar":
          pool = products.filter((p) => p.id !== source.id && p.categoryId === source.categoryId);
          break;
        case "complete-the-look":
          ...
          break;
        default:
          pool = products.filter((p) => p.id !== source.id && p.gender === source.gender);
          break;
      }
